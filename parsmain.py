...
def moslogin(login, password):
    ...
    firefox_driver = webdriver.Firefox(executable_path="C:\\Users\\PCWORK\Desktop\\alter\AlterGUI\\geckodriver.exe", options = firefox_options)
    ...
    try:
        ...
        s.get(f'https://lk.emias.mos.ru/api/1/documents/inspections?ehrId={idus}&shortDateFilter=all_time')
        polic = s.driver.execute_script("return window.sessionStorage.getItem('profile/profileData')")
        print(json.loads(polic))
    s.driver.quit()
